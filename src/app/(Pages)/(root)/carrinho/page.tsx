...
async function page() {
   const cart = await getCart()
    ...
    const produtos = cart.length>0?cart.reduce((acc, item) => acc + item.quantity, 0):null;

    function getTotal() {
     const total = cart.length>0?cart.reduce((acc, item) => +item.product.price.replace(",", ".").replace("R$", "") * item.quantity + acc, 0).toFixed(2):null;
     return total;
   }
  ...
  return (
    <section className='flex-1  lg:grid lg:gap-5 max-w-[1440px]  lg:grid-cols-3 mx-auto mt-20 sm:mt-5 mb-5 px-5'>
        <div className='col-span-2'>
            <h1 className='font-alex md:text-title text-titleMobile  text-mainTitle'>{cart.length>0?"Carrinho":"Carrinho Vazio"}</h1>
            < CartItems cart={cart} />
        </div>
        {cart.length>0?<div className='lg:mt-[150px] mt-[40px] w-full flex lg:flex-nowrap flex-wrap justify-center mx-auto  lg:flex-col gap-10'>
            <div  className='bg-white border-[1px] globalShadow border-[#eeeeee] min-w-full  max-h-[215px] h-full rounded-md'>
            ...
            <div className='bg-white border-[1px] globalShadow border-[#eeeeee] flex flex-col gap-2 p-5 min-w-full max-h-[215px] h-full rounded-md'>
