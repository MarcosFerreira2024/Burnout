...
function NavButton({onClick,label,href,src="/ui/perfil.svg",alt,perfil,classes, ...props}:NavButton) {
  ...
  useEffect(()=>{
    ...
    if(href && typeof href === "string"){
      ...
      return(
        <Link {...props} href={href} ref={buttonRef} className={` ${classes} flex outline-mainSubtitle relative  hover:opacity-90 hover:bg-mainStroke transition-opacity  ease-in-out duration-300 border-[1px] border-mainStroke gap-2    ${modal ?" animaInNavBarButtons  ":(`${mounted?"animaOutNavBarButtons":""} xl:justify-normal  xl:pl-2`)} w-[100%] xl:max-w-[160px] font-poppins py-1 min-h-[32px]  max-h-[32px] max-w-[32px] items-center bg-mainBg inner drop rounded-md    `}>
